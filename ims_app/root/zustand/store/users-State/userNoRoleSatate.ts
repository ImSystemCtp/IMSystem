    ...
    return {
        ...
        getNextPage: async () => {
            if (get().haveNextPage && get().countPage == get().pagine) {
                const usersPending = await userProvider.getUsers({ offset: get().cursorPending, limit: LIMIT, filterBy: "usu_role", filterCondition: "equals", filterValue: "noRole" } as QueryOptions);
