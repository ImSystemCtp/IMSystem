...
export default function RegisterLocation() {
    const {addLocation,locationToEdit} = useLocationStore();
    const name = locationToEdit ? locationToEdit.location_name : "" ;
    let initialValues: FormValues = {
        location_name: name
    };
    const handleSubmit = async (values: FormValues) => {
        const {location_name} = values;
        addLocation({location_name} as ims_locations)
    };
    return (
        <main className=" ">
                    ...
                    <Form className="border-2  rounded-lg border-slate-300 shadow-sm shadow-slate-300 p-4 w-3/4">
                        ...
                        <div className="w-full mt-4 flex justify-center">
                            <motion.button
                                ...
                                {locationToEdit ? "Guardar Cambios":"Registrar"}
                            </motion.button>
                        </div>
                    </Form>
                </div>
